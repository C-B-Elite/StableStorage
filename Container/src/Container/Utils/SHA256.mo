...
module {
  ...
  public func sha256(data_ : [Blob]) : [Nat] {
    let data = Prim.Array_tabulate<Blob>(data_.size(), Prim.blobToArray);
    let digest = Digest();
    digest.write(data);
    return digest.sum();
  };
  ...
  public class Digest() {
    ...
    public func sum() : [Nat] {
      ...
      write(Array.freeze<Nat8>(buf));
      let hash = Array.init<Nat>(32, 0);
      for (i in Iter.range(0, 7)) {
        for (j in Iter.range(0, 3)) {
          let k : Nat32 = 24 -% 8 *% Nat32.fromIntWrap(j);
          hash[4 * i + j] := Nat32.toNat(s[i] >> k);
        };
      };
      return Array.freeze<Nat>(hash);
    };
